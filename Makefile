...
PLUMBER = github.com/zanecloud/plumber

VERSION = $(shell cat VERSION)
GITCOMMIT = $(shell git log -1 --pretty=format:%h)
BUILD_TIME = $(shell date --rfc-3339 ns 2>/dev/null | sed -e 's/ /T/')

IMAGE_NAME = registry.cn-hangzhou.aliyuncs.com/zanecloud/plumber

build:
	docker run -v $(shell pwd):/go/src/${PLUMBER} -w /go/src/${PLUMBER} --rm ${GOLANG} make build-local
...
build-local:
	...
	CGO_ENABLED=0 go build -v -ldflags "-X main.Version=${VERSION} -X main.GitCommit=${GITCOMMIT} -X main.BuildTime=${BUILD_TIME}" -o bundles/${VERSION}/binary/plumber ${PLUMBER}
